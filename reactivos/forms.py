...
class CustomPasswordResetForm(PasswordResetForm):
    captcha = CaptchaField(
        widget=CaptchaTextInput(attrs={"class": "form-control", "id": "recaptcha"}),
...
class FormularioUsuario(forms.ModelForm):
    ...
    acceptDataProcessing = forms.BooleanField(
        required=True,
    ...
    class Meta:
        model = User
        fields = ("first_name", "last_name","position_company")
        widgets = {
            ...
            "position_company": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "Cargo",
                    "id": "position_company",
                    "required": "required",
                    "maxlength": "150",  # Agregado
                    "pattern": "^.{1,150}$",  # Modificado para aceptar de 1 a 150 caracteres
                    "title": "El cargo debe ser de máximo 150 caracteres",
                }
            ),
        }
    ...
    def clean(self):
        ...
        cleaned_data["position_company"] = estandarizar_nombre(cleaned_data["position_company"])
        return cleaned_data
    ...
    def save(self, commit=True):
        ...
        )  # Usar 'pasword1' en lugar de 'password1'
        if commit:
            user.save()
        return user
...
class ConfiguracionSistemaForm(forms.ModelForm):
    ...
    def clean(self):
        cleaned_data = super().clean()
        cleaned_data["correo_administrador"] = estandarizar_nombre(
            cleaned_data["correo_administrador"]
        )
...
class SolicitudesExternasForm(forms.ModelForm):
    ...
    access_token = forms.CharField(
        widget=forms.HiddenInput(attrs={"id": "access_token"}), required=False
    )
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "title": "Nombres y apellidos que incluyan solo letras con máximo de 50 caracteres",
                    "autocomplete": "off",
                }
            ),
            "subject": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "title": "El asunto de la solicitud debe tener un máximo de 100 caracteres",
                    "autocomplete": "off",
                }
            ),
            "message": forms.Textarea(
                attrs={
                    "class": "form-control",
                    "title": "El texto que describe la solicitud debe tener un máximo de 1000 caracteres",
                    "autocomplete": "off",
                }
            ),
            "attach": forms.FileInput(
                attrs={"title": "Adjuntar archivo (Opcional)", "autocomplete": "off"}
            ),
            "lab": forms.Select(
                attrs={
                    "class": "form-control",
                    "title": "Seleccione un laboratorio, este campo es obligatorio",
                    "autocomplete": "off",
                }
            ),
            "email": forms.EmailInput(
                attrs={
                    "class": "form-control",
                    "title": "Ingrese un correo electrónico válido de la Universidad Nacional de Colombia (XXX@unal.edu.co)",
                    "readonly": "readonly",
                    "autocomplete": "off",
                }
            ),
            "mobile_number": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "title": "Ingrese un número de móvil válido en el rango de 3000000000 a 3999999999",
                    "autocomplete": "off",
                }
            ),
            "department": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "title": "Ingrese el departamento correspondiente (máximo 100 caracteres)",
                    "autocomplete": "off",
                }
            ),
            "accept_politics": forms.CheckboxInput(
                attrs={
                    "title": "Debe aceptar la política de tratamiento de datos personales",
                    "unchecked": "unchecked",
                    "required": "required",
                }
            ),
    ...
    def clean_email(self):
        ...
        if not email.endswith("@unal.edu.co"):
            raise forms.ValidationError(
                "El correo electrónico debe tener el dominio @unal.edu.co."
            )
        return email
    ...
    def clean_mobile_number(self):
        ...
        if not (3000000000 <= int(mobile_number) <= 3999999999):
            raise forms.ValidationError(
                "El número de móvil debe estar entre 3000000000 y 3999999999."
            )
        ...
        if attach:
            ...
            if attach.size > 5 * 1024 * 1024:
                raise ValidationError(
                    "El tamaño del archivo adjunto debe ser como máximo de 5 MB."
                )
        return attach
